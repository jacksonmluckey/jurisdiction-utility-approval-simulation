...
## Using the City Class (Recommended)

The `City` class provides a unified interface for creating cities with all configuration in one place.

### Simple Example

```{python}
# Create a basic city with default settings
city_config = CityConfig(
    width=60,
    height=60,
    block_area_acres=2.47,  # ~100m x 100m blocks
...
city.visualize(show=True)
```

### City with Transportation Corridors
...
city_config_transport = CityConfig(
    width=70,
    height=70,
...
polycentric_config_transport = PolycentricConfig(
    num_centers=5,
    center_distribution="uniform",
...
city_with_transport = City(
    config=city_config_transport,
    polycentric_config=polycentric_config_transport,
    transport_config=transport_config
)
grid_transport = city_with_transport.generate()

city_with_transport.summary()
city_with_transport.visualize(show=True)
```
